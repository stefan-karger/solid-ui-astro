...
With `solid-charts`, axes are declared explicitly, so we'll start with the chart scales in place and then layer on the rest.
...
To add a visible baseline to the x-axis, we'll use the `AxisLine` component.

<Steps class="mb-0 pt-2">

<Step>Import the `AxisLine` component.</Step>

```tsx /AxisLine/
import { AxisLine } from "solid-charts"
```

<Step>Add the `AxisLine` component to your chart.</Step>
