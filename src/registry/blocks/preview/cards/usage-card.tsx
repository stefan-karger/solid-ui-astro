...
const items = [
  ...
  { name: "ISR Writes", value: "524.52K / 2M", percentage: 26.23 },
  { name: "Function Duration", value: "5.11 GB Hrs / 1K GB Hrs", percentage: 5.11 }
]

function CircularGauge(props: { percentage: number }) {
  const circumference = 2 * Math.PI * 42.5
  const strokePercent = createMemo(
    () => (Math.min(Math.max(props.percentage, 0), 100) / 100) * circumference
  )
